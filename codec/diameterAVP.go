...
// Generate a simplified struct suitable for JSON encoding
// with only "name" and "value" attributes
func (avp DiameterAVP) ToSimple() map[string]interface{} {
	theMap := map[string]interface{}{"name": avp.Name}
	if avp.Group == nil {
		// Simple AVP
		avpType := avp.DictItem.DiameterType
		if avpType == diamdict.Integer32 || avpType == diamdict.Integer64 || avpType == diamdict.Unsigned32 || avpType == diamdict.Unsigned64 {
			theMap["value"] = avp.LongValue
		} else if avpType == diamdict.Float32 || avpType == diamdict.Float64 {
			theMap["value"] = avp.FloatValue
		} else {
			theMap["value"] = avp.StringValue
		}
	} else {
		// Grouped AVP.The value is an array of JSON
		theGroup := make([]map[string]interface{}, 0)
		for i := range avp.Group {
			theGroup = append(theGroup, avp.Group[i].ToSimple())
		}
		theMap["value"] = theGroup
	}

	return theMap
}

// Encode as JSON using the simplified representation
func (avp DiameterAVP) MarshalJSON() ([]byte, error) {
	return json.Marshal(avp.ToSimple())
}

// Generates a DiameterAVP from an dynamically unserialized JSON with
// only "name" and "value" attributes
func FromSimple(simpleAVP map[string]interface{}) (DiameterAVP, error) {
	avp := DiameterAVP{}
	name, ok := simpleAVP["name"]
	if !ok {
		return avp, fmt.Errorf("name not found in JSON")
	}

	avp.SetName(name.(string))
	value, ok := simpleAVP["value"]
	if !ok {
		return avp, fmt.Errorf("value not found in JSON")
	}

	switch v := value.(type) {
	case []interface{}:
		group := make([]DiameterAVP, 0)
		for _, innerValue := range v {
			innerAVP, err := FromSimple(innerValue.(map[string]interface{}))
			if err != nil {
				return avp, err
			} else {
				group = append(group, innerAVP)
			}
		}
		avp.Group = group

	case string:
		avp.SetStringValue(v)

	case float64:
		avp.SetFloatValue(v)

	case int:
		avp.SetLongValue(int64(v))

	default:
		config.IgorLogger.Errorf("Unknow type when umarshalling from JSON", v)
		return avp, nil
	}
