...
// Uses the Actor model. All interaction must happen through
// the channels
...
func NewRadiusSessionStore(idAttributes []string, indexNames []string, expirationTime time.Duration, limboTime time.Duration) *RadiusSessionStore {
	...
	// Default index
	store.sessions = make(map[string]*RadiusSessionEntry)
	// Additional indexes
	store.indexes = make(map[string]map[string]map[string]struct{})
	for _, indexName := range indexNames {
		store.indexes[indexName] = make(map[string]map[string]struct{})
	}
